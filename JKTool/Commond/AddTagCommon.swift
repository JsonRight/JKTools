...
class AddTagCommon: PullCommon {
    override func run(options: ConsoleOptions) {
        ...
        if pro.rootProject != pro {
            self.addTag(pro: pro.rootProject, options: options)
        }
        do {
            ...
            for record in recordList {
                ...
                self.addTag(pro: pro1, options: options)
            }
        } catch {
    ...
    func addTag(pro: Project,options: ConsoleOptions) {
