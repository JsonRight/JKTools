        ...
        mutating func run() {
            ...
            func push(project: Project){
                ...
                guard let branch = branch else {
                    return po(tip: "【\(project.workSpaceType.projectName())】 Push失败\n" + "无法检索出当前分支名",type: .error)
                }
                
                do {
                    try shellOut(to: .gitPush(branch: branch), at: project.directoryPath)
                    po(tip: "【\(project.workSpaceType.projectName())】Push[\(branch)]完成", type: .tip)
                } catch {
                    let error = error as! ShellOutError
                    po(tip: "【\(project.workSpaceType.projectName())】 Push[\(branch)]失败\n" + error.message + error.output,type: .warning)
                }
            }
